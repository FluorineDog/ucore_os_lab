...
typedef struct {
	...
	struct proc_struct *
			proc;	//the proc wait in this timer. If the expire time is end, then this proc will be scheduled
...
// call scheduler to update tick related info,
// and check the timer is expired? If expired, then wakup proc
