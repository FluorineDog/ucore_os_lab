...
static int _assert(BOOLEAN condition,
									 int line); /* for line, pass in __LINE__ */
...
void memoryDump() {
   ...
   */
     ...
     */
		...
		printf(
				"Block %d, %3zd B (%3zd B) at %p (after header), is %s\n", /* replace the size_t prints %d by %zd on Ubuntu */
				++blockCounter, b->size, withHeader, b->data,
   ...
   */
	...
	printf(
			"------------- ending memory dump :heap content -----------------\n\n");
}
...
void _merge(BlockPtr b, BlockPtr nextBlock) {
	...
	_assert(b->isFree && nextBlock->isFree,
					__LINE__);											 /* both blocks think they are free */
	_assert(b->next == nextBlock, __LINE__); /* b1 thinks that b2 is after her */
	_assert(b == nextBlock->previous,
					__LINE__);								/* b2 thinks that b1 is before him */
...
BOOLEAN _isValidAddress(void *ptr) {
	...
	if (p) {
		isValid = p <= g_heapsEnd && p >= g_heapsBase;
	}
	return isValid;
}
...
static int _assert(BOOLEAN condition, int line) {
	if (!condition) {
		fprintf(stderr, "_assertION FAILURE: at %s, line %d\n", __FILE__, line);
		return FALSE;
	}
	return TRUE;
}
